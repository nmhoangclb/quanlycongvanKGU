<?php
	...
	$result = mysqli_query($conn, "SELECT count(idcongvan) as total FROM congvan, taptin WHERE congvan.mataptin = taptin.id AND hinhthucvanban = 15");
	...
	$result = mysqli_query($conn, "SELECT * FROM congvan, taptin, hinhthucvanban, coquanbanhanh WHERE hinhthucvanban = 15 AND congvan.mataptin = taptin.id AND congvan.hinhthucvanban = hinhthucvanban.id AND congvan.coquanbanhanh = coquanbanhanh.id ORDER BY ngayVanBan DESC LIMIT $start, $limit");//Desc giảm dần Asc tăng dần
	?>
<div>
	<?php 
...
<div class="table-page">
   <?php 
    ...
    if ($current_page > 1 && $total_page > 1){
        echo '<a class="page-prev" href="thongbao.php?page='.($current_page-1).'">Trang trước</a>';
    }
    ...
    for ($i = 1; $i <= $total_page; $i++){
        ...
        if ($i == $current_page){
            ...
            echo '<a class="page-number" href="thongbao.php?page='.$i.'">'.$i.'</a>';
        }
    }

    // nếu current_page < $total_page và total_page > 1 mới hiển thị nút prev
    if ($current_page < $total_page && $total_page > 1){
        echo '<a class="page-next" href="thongbao.php?page='.($current_page+1).'">Trang sau</a>';
    }
