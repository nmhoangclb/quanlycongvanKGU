...
$result = mysqli_query($conn, "SELECT count(idcongvan) as total FROM congvan, taptin WHERE congvan.mataptin = taptin.id AND coquanbanhanh = 1");
...
$result = mysqli_query($conn, "SELECT * FROM congvan, taptin, hinhthucvanban, coquanbanhanh WHERE coquanbanhanh = 1 AND congvan.mataptin = taptin.id AND congvan.hinhthucvanban = hinhthucvanban.id AND congvan.coquanbanhanh = coquanbanhanh.id ORDER BY ngayVanBan DESC LIMIT $start, $limit"); //Desc giảm dần Asc tăng dần
?>
<div>
	<?php
